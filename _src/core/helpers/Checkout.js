...
import {onEncrypt} from '@app/helpers/Encryption';
...
export const getPwaUrl = ({token, cartId}) => {
  const urlPath = '/';
  const getToken = token === null ? '' : token;
  const data = `${getToken}|${cartId}|${urlPath}`;
  const pwaUrl = Config.PWA_CHECKOUT_URL;
  const pwaKey = Config.PWA_CHECKOUT_KEY;
  const getEncrypt = onEncrypt({data, key: pwaKey});
  const getEncodeUri = encodeURIComponent(getEncrypt);
  const pwaUrlFinal = pwaUrl + 'authentication?state=' + getEncodeUri;
  return pwaUrlFinal;
};
