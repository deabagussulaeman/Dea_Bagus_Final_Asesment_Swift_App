...
import AtomBlockShippingAddress from '@app/_modules/cart_checkout/atoms/BlockShippingAddress';
import AtomBlockShippingMethod from '@app/_modules/cart_checkout/atoms/BlockShippingMethod';
import AtomBlockPaymentMethod from '@app/_modules/cart_checkout/atoms/BlockPaymentMethod';
import AtomBlockPaymentOthers from '@app/_modules/cart_checkout/atoms/BlockPaymentOthers';
import styles from '@app/_modules/cart_checkout/views/tablet/styles';
import AppModal from '@app/components/_AppModal';
import Icon from 'react-native-vector-icons/Feather';
import _ from 'lodash';

function CartCheckoutTabletView({
  t,
  cart,
  isCart,
  isLoadingSnap,
        ...
        <Section horizontalStart flexNumber={3}>
          <Section width={Mixins.MAX_WIDTH - 40} row spaceBetween>
            ...
            <Show when={item.product.__typename !== TYPENAME_BUNDLE}>
              <Label small style={[styles.cartItemDetailText]}>
                {isFreeItem
  ...
  const SubTotal = () => {
    return (
      <Section
        ...
        <Show when={!expand}>
          <Section marginBottom={5} onPress={onPressExpand}>
            <Icon
              name={'chevron-up'}
              size={25}
              color={
        ...
        <Show
          ...
          <Section marginVertical={10}>
            <Label small bold>{`Total ${numberFormat(
              dataPrices.grand_total.currency,
              dataPrices.grand_total.value,
            )}`}</Label>
          </Section>
        </Show>
        <Show
...
export default CartCheckoutTabletView;
